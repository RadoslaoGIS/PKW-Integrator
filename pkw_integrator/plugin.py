...
class Dialog(QDialog, Ui_Dialog):
    ...
    def generate(self):
        ...
        try:
            ...
            csv_data, headers, excluded_columns = self.prepare_csv_data(csv_data, headers, unit_type)
            ...
            # Integracja danych
            synchronized_count = self.integrate_data(csv_data, shapefile_path, unit_type, excluded_columns)
    ...
    def prepare_csv_data(self, csv_data, headers, unit_type):
        """Przygotowanie danych CSV - dodanie kodów TERYT i identyfikacja kolumn do wykluczenia"""
        ...
        if unit_type == 'wojewodztwa' and not teryt_column:
            ...
            if voivodeship_column:
                # Dodanie kolumny z kodami TERYT do każdego wiersza
                for row in csv_data:
                ...
                # Dodaj nagłówek "Kod TERYT" jeśli nie istnieje
                if 'Kod TERYT' not in headers:
                    headers.append('Kod TERYT')

        # Utwórz listę kolumn do wykluczenia z integracji
        excluded_columns = []

        # Dla województw wykluczamy kolumnę z nazwami województw
        if unit_type == 'wojewodztwa':
            for header in headers:
                if 'województwo' in header.lower():
                    excluded_columns.append(header)

        # Dla powiatów wykluczamy kolumny z województwami i powiatami
        elif unit_type == 'powiaty':
            for header in headers:
                if any(term in header.lower() for term in ['województwo', 'powiat']):
                    excluded_columns.append(header)

        # Dla gmin wykluczamy kolumny z województwami, powiatami i gminami
        elif unit_type == 'gminy':
            for header in headers:
                if any(term in header.lower() for term in ['województwo', 'powiat', 'gmina']):
                    excluded_columns.append(header)

        print(f"Kolumny do wykluczenia: {excluded_columns}")
        return csv_data, headers, excluded_columns
    ...
    def integrate_data(self, csv_data, shapefile_path, unit_type, excluded_columns=None):
        ...
        for feature in layer.getFeatures():
            if len(sample_shp_codes) < 5:
                ...
                break
        print(f"Przykładowe kody TERYT z shapefile: {sample_shp_codes}")
        ...
        # Dodanie nowych pól z CSV (oprócz wykluczonych kolumn)
        new_fields = []
        
        # Utworzenie listy wszystkich wykluczonych kolumn
        all_excluded = excluded_columns + [teryt_column_csv] if teryt_column_csv else excluded_columns[:]
        
        # Filtrowanie nagłówków CSV - wykluczamy niepotrzebne kolumny
        csv_data_headers = [h for h in csv_headers if h not in all_excluded]
        
        print(f"Wszystkie nagłówki CSV: {csv_headers}")
        print(f"Wykluczone kolumny: {all_excluded}")
        print(f"Kolumny do integracji: {csv_data_headers}")
        ...
        for header in csv_data_headers:
            ...
            if not field_exists:
                # Sprawdź typ danych w pierwszym wierszu, aby określić typ pola
                sample_value = str(csv_data[0].get(header, '')) if csv_data else ''
                
                # Próbuj określić czy to liczba czy tekst
                if sample_value.replace('.', '').replace(',', '').replace('-', '').isdigit():
                    # To prawdopodobnie liczba
                    new_fields.append(QgsField(header, QVariant.Int, len=10))
                else:
                    # To tekst
                    new_fields.append(QgsField(header, QVariant.String, len=255))

        print(f"Nowe pola do dodania: {[f.name() for f in new_fields]}")
        ...
        print(f"Mapowanie nazw pól: {list(field_name_to_index.keys())}")
        ...
        for original_feature in layer.getFeatures():
            ...
            if teryt_code_shp in csv_dict:
                ...
                # Wypełnienie nowych pól danymi z CSV (tylko te, które nie są wykluczone)
                for header in csv_data_headers:
        ...
        print(f"Zsynchronizowano: {synchronized_count} jednostek z {len(csv_dict)} dostępnych w CSV")
